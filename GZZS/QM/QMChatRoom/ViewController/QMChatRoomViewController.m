...
- (void)createUI {
    ...
    // 扩展面板
    self.addView = [[QMChatRoomMoreView alloc] initWithFrame:CGRectMake(0, kScreenHeight, kScreenWidth, 110)];
    ...
    [self.logoutButton setTitle:@"返回" forState:UIControlStateNormal];
...
- (void)keyboardFrameChange: (NSNotification *)notification {
    ...
    if (newFrame.origin.y == kScreenHeight) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            self.chatInputView.frame = CGRectMake(0, kScreenHeight-kInputViewHeight-newFrame.size.height-_navHeight, kScreenWidth, kInputViewHeight);
            self.chatTableView.frame = CGRectMake(0, 0, kScreenWidth, kScreenHeight-_navHeight-kInputViewHeight-newFrame.size.height);
            [self scrollToEnd];
        }];
    }
}
